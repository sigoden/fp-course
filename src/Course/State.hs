...
instance Applicative (State s) where
  ...
  (<*>) ::
    ...
    -> State s a
    -> State s b 
  (<*>) (State f) (State g) = State (\x -> let (f', s1) = f x
                                               (a, s2) = g s1
                                           in (f' a, s2))
...
findM ::
  ...
  -> List a
  -> f (Optional a)
findM f = foldRight (\a as -> lift2 (\b as' -> if b then Full a else as') (f a) as) (return Empty)
