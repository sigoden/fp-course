...
tok ::
  Parser a
  -> Parser a
tok a = (\a' -> (\_ -> valueParser a') =<< spaces) =<< a
...
between ::
  ...
  -> Parser a
  -> Parser a
between a b c = a >>= (\_ -> c >>= (\c' -> b >>= (\_ -> valueParser c')))
...
hex ::
  Parser Char
hex = replicateA 4 (satisfy isHexDigit) >>= (\v -> case readHex v of
...
sepby1 ::
  ...
  -> Parser s
  -> Parser (List a)
sepby1 a s = a >>= (\b -> list (s >> a) >>=  (\c -> valueParser (b:.c)))
...
eof ::
  Parser ()
eof = P (\i -> if i == Nil then Result i () else ErrorResult (ExpectedEof i))
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll ls = character >>= (\c -> if all (\f -> f c) ls then valueParser c else failed)
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny ls = character >>= (\c -> if any (\f -> f c) ls then valueParser c else failed)
