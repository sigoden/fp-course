...
findLeft f (ListZipper l c r) = case break f l of
                                  (_, Nil) -> IsNotZ
                                  (a, h:.t) -> IsZ (ListZipper t h (a ++ (c:.Nil) ++ r))
...
findRight f (ListZipper l c r) = case break f r of
                                   (_, Nil) -> IsNotZ
                                   (a, h:.t) -> IsZ (ListZipper (reverse a ++ (c:.l)) h t)
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper Nil c r) = let h:.t = reverse (c:.r)
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper l c Nil)= let h:.t = reverse l ++ (c:.Nil)
...
-- prop> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end (ListZipper l c r) = let (h:.t) = reverse (c:.r) ++ l
                        in ListZipper t h Nil
...
start ::
  ListZipper a
  -> ListZipper a
start (ListZipper l c r) = let (h:.t) = reverse l ++ (c:.Nil) ++ r
                           in ListZipper Nil h t
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper l1 c1 r1) (ListZipper l2 c2 r2) = ListZipper (zipWith (\f a -> f a) l1 l2) (c1 c2) (zipWith (\f a -> f a) r1 r2)
...
instance Extend ListZipper where
    (<<=) f lz = ListZipper (unfoldr (\x -> case moveLeft x of
                                              IsZ lz' -> Full (f lz', lz')
                                              IsNotZ -> Empty) lz) (f lz) (unfoldr (\x -> case moveRight x of
                                                                                        IsZ lz' -> Full (f lz', lz')
                                                                                        IsNotZ -> Empty
                                                                                        ) lz)
